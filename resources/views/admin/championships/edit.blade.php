<x-app-layout>
    <div class="mx-auto max-w-2xl px-4 py-10 sm:px-6 lg:px-8">
        <div class="mb-8">
            ...
            <h1 class="text-2xl font-extrabold text-white">Editar Campeonato</h1>
        </div>
        @if($errors->any())
        ...
        <div class="rounded-2xl border border-slate-700 bg-slate-900 p-6">
            <form method="POST" action="{{ route('admin.championships.update', $championship) }}" class="space-y-6">
                @csrf
                @method('PATCH')
                ...
                <div class="flex items-center justify-between border-t border-slate-800 pt-6">
                    <form method="POST" action="{{ route('admin.championships.destroy', $championship) }}"
                        onsubmit="return confirm('Remover {{ $championship->name }}?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
